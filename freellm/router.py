...
Usage:
    python router.py --llm-port 20100 --router-port 20200
"""
...
FREELLM_PORT = 20100
ROUTER_PORT = 20200
FREELLM_URL = f"http://127.0.0.1:{FREELLM_PORT}"
_server_process = None

FREE_MODELS = {
    "glm-5-free": {"id": "glm-5-free", "name": "GLM 5 Free", "provider": "freellm"},
    "kimi-k2.5-free": {"id": "kimi-k2.5-free", "name": "Kimi K2.5 Free", "provider": "freellm"},
    "minimax-m2.5-free": {"id": "minimax-m2.5-free", "name": "MiniMax M2.5 Free", "provider": "freellm"},
    "gpt-5-nano": {"id": "gpt-5-nano", "name": "GPT-5 Nano", "provider": "freellm"},
    "big-pickle": {"id": "big-pickle", "name": "Big Pickle", "provider": "freellm"},
}


def start_freellm_server():
    global _server_process
    
    if is_server_running():
        return
    
    cmd = ["opencode", "serve", "--port", str(FREELLM_PORT), "--hostname", "127.0.0.1"]
...
def is_server_running() -> bool:
    try:
        resp = requests.get(f"{FREELLM_URL}/global/health", timeout=2)
...
@app.get("/health")
async def health():
    return {"status": "ok", "freellm_running": is_server_running(), "llm_port": FREELLM_PORT, "router_port": ROUTER_PORT}
...
async def get_model(model_id: str):
    ...
    if model_id.startswith("freellm/"):
        model_id = model_id[8:]
...
async def chat_completions(
    ...
    session_resp = requests.post(f"{FREELLM_URL}/session", json={})
    ...
    resp = requests.post(
        f"{FREELLM_URL}/session/{session_id}/message",
        json=body
    )
...
class OpenAIRouterService:
    ...
    def start_router(
        ...
        cmd = [
            ...
            "freellm.router:create_app_factory",
            "--host", host,
        ...
        try:
            self._process = subprocess.Popen(
                ...
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
                env={**os.environ, "FREELLM_URL": llm_url}
            )
...
def create_app_factory():
    """创建 FastAPI 应用工厂（用于 uvicorn）"""
    llm_url = os.environ.get("FREELLM_URL", f"http://127.0.0.1:{FREELLM_PORT}")
...
def run_router_standalone(llm_port: int = 20100, router_port: int = 20200, host: str = "0.0.0.0"):
    """独立运行 Router 服务"""
    import uvicorn
    
    global FREELLM_URL, FREELLM_PORT, ROUTER_PORT
    FREELLM_PORT = llm_port
    ROUTER_PORT = router_port
    FREELLM_URL = f"http://127.0.0.1:{llm_port}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--llm-port", type=int, default=20100,
                        help="Port for LLM serve (default: 20100)")
    parser.add_argument("--router-port", type=int, default=20200,
                        help="Port for router API server (default: 20200)")
    ...
    FREELLM_PORT = args.llm_port
    ROUTER_PORT = args.router_port
    FREELLM_URL = f"http://127.0.0.1:{FREELLM_PORT}"
    
    print(f"[router] Starting with llm_port={FREELLM_PORT}, router_port={ROUTER_PORT}")
    
    run_router_standalone(FREELLM_PORT, ROUTER_PORT, args.host)
