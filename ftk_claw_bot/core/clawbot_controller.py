...
from ..models import ClawbotConfig, ClawbotStatus, ClawbotInstance
...
class ClawbotController:
    def __init__(self, wsl_manager: WSLManager):
        ...
        self._instances: dict[str, ClawbotInstance] = {}
    ...
    def start(self, config: ClawbotConfig) -> bool:
        ...
        if config.apiKey:
            self._write_clawbot_config(config)

        args = config.to_clawbot_args()
        cmd = ["clawbot"] + args

        instance = ClawbotInstance(
            config=config,
            status=ClawbotStatus.STARTING,
            started_at=datetime.now()
        )
        self._instances[config.name] = instance

        thread = threading.Thread(
            target=self._run_clawbot,
    ...
    def _write_clawbot_config(self, config: ClawbotConfig):
        config_content = config.to_full_clawbot_config()
    ...
    def _run_clawbot(self, distro_name: str, cmd: List[str], instance_name: str):
        ...
        try:
            ...
            instance.status = ClawbotStatus.RUNNING
            ...
            instance.status = ClawbotStatus.STOPPED
            instance.pid = None
            self._notify_status(instance_name)

        except Exception as e:
            instance.status = ClawbotStatus.ERROR
    ...
    def get_instance(self, config_name: str) -> Optional[ClawbotInstance]:
        return self._instances.get(config_name)

    def get_status(self, config_name: str) -> Optional[ClawbotStatus]:
        instance = self._instances.get(config_name)
        return instance.status if instance else None

    def is_running(self, config_name: str, check_connectivity: bool = False) -> bool:
        """Check if clawbot instance is running.
        ...
        """
        instance = self._instances.get(config_name)
        if instance is None or instance.status != ClawbotStatus.RUNNING:
            return False
    ...
    def _check_websocket_connectivity(self, instance: ClawbotInstance) -> bool:
        """Check if clawbot instance is reachable via WebSocket.

        Args:
            instance: The clawbot instance to check
    ...
    def get_logs(self, config_name: str, lines: int = 100) -> List[str]:
        """Get logs for a specific clawbot instance.
    ...
    def install_systemd_service(self, distro_name: str) -> bool:
        """Install clawbot as systemd service in WSL2."""
        try:
            service_content = """[Unit]
Description=Clawbot Gateway Service
After=network.target

[Service]
Type=simple
User=%I
ExecStart=/usr/bin/clawbot gateway
Restart=always
RestartSec=10
...
"""
            result = self._wsl_manager.execute_command(
                distro_name,
                f"cat > /etc/systemd/system/clawbot-gateway.service << 'EOF'\n{service_content}\nEOF"
            )
            if not result.success:
                return False

            self._wsl_manager.execute_command(distro_name, "sudo systemctl daemon-reload")
            self._wsl_manager.execute_command(distro_name, "sudo systemctl enable clawbot-gateway")
            return True
        except Exception:
            return False

    def uninstall_systemd_service(self, distro_name: str) -> bool:
        """Uninstall clawbot systemd service."""
        try:
            self._wsl_manager.execute_command(distro_name, "sudo systemctl stop clawbot-gateway")
            self._wsl_manager.execute_command(distro_name, "sudo systemctl disable clawbot-gateway")
            self._wsl_manager.execute_command(
                distro_name, "sudo rm -f /etc/systemd/system/clawbot-gateway.service"
            )
            self._wsl_manager.execute_command(distro_name, "sudo systemctl daemon-reload")
            return True
        except Exception:
    ...
    def is_systemd_service_installed(self, distro_name: str) -> bool:
        ...
        result = self._wsl_manager.execute_command(
            distro_name, "test -f /etc/systemd/system/clawbot-gateway.service"
        )
        return result.success
    ...
    def sync_config_to_wsl(self, config: ClawbotConfig) -> bool:
        ...
        if restart_result.success:
            logger.info(f"✓ clawbot 服务重启成功: {config.distro_name}")
        else:
