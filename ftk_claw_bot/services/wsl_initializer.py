...
class WSLInitializer:
    ...
    def initialize_distro(
        ...
        steps = [
            ...
            ("安装 nanobot", self._install_nanobot, (whl_path,)),
        ]

        if not skip_opencode:
            steps.append(("安装 OpenCode", self._install_opencode, ()))
...
"""
    ...
    def _install_nanobot(self, whl_path: str) -> Tuple[bool, str]:
        if not os.path.exists(whl_path):
            return False, f"Wheel 文件不存在: {whl_path}"

        whl_name = os.path.basename(whl_path)
        self._emit_log(f"安装 nanobot: {whl_name}")
